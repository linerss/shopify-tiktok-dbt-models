...
line_items_raw as (
    ...
    select
        ...
        -- Line item fields (from parsed JSON)
        o.id::int::string as order_id,
        parsed.value:"id"::int::string as line_item_id,  -- Cast to int first to normalize decimal formats (e.g., 123.000000000 -> 123)
    ...
    from {{ source('shopify', b.source) }} o,
         lateral flatten(input => try_parse_json(o.line_items)) as parsed
    {% if is_incremental() %}
    where o.updated_at >= (
          select max_date
          from max_timestamps
    ...
    from (
        select
            ...
            row_number() over (
                ...
                order by daton_batch_runtime desc, daton_batch_id desc
            ) as rn
        from order_notes
    )
    where rn = 1
